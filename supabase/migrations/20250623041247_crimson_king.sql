...
-- Improved function to find matches in the queue - FIXED for DISTINCT/ORDER BY issue
...
BEGIN
  ...
  -- Look for compatible pairs in the queue - FIXED: include ORDER BY expressions in SELECT
  FOR match_pair IN
    SELECT DISTINCT
      ...
      GREATEST(amq1.queued_at, amq2.queued_at) as latest_queue_time  -- Added to SELECT
