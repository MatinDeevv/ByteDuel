...
-- Fixed function to find matches in the queue (removed DISTINCT to avoid ORDER BY issues)
...
BEGIN
  ...
  -- Look for compatible pairs in the queue (removed DISTINCT and fixed ORDER BY)
  FOR match_pair IN
    SELECT 
      ...
      GREATEST(amq1.queued_at, amq2.queued_at) as latest_queue_time
    ...
    ORDER BY 
      rating_diff ASC,  -- Closest ratings first
      latest_queue_time ASC  -- Longest waiting players first
...
-- Function to manually trigger a match check (for debugging) - FIXED
...
DECLARE
  ...
  match_result jsonb;
BEGIN
  -- Get current queue status with fixed ordering
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      'current_rating_range', current_rating_range,
      'queued_at', queued_at,
