...
<invoke name="file" filePath="src/pages/DuelPage.tsx">
...
const DuelPage: React.FC = () => {
  ...
  const { user } = useAuth();
  const { userRating, setUserRating, clearMatch } = useAdvancedMatchmakingStore();
    ...
    const cleanupQueue = async () => {
      if (user?.id) {
        try {
          ...
          clearMatch();
          console.log('🧹 Cleaned up user from advanced queue on duel page load');
        } catch (error) {
